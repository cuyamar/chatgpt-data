...
public abstract class AbstractAuthService implements IAuthService{
    ...
    private static String defaultBase64EncodedSecretKey = "B*B^D%fe";
    ...
    public AuthStateEntity doLogin(String code) {
        //1.如果不是4位有效数字则返回验证码无效
        if(!code.matches("\\d{4}")){
            log.info("鉴权，用户收到的验证码无效{}",code);
        ...
        //2.校验判断，非成功则直接返回
        AuthStateEntity authStateEntity = checkCode(code);
    ...
    */
    ...
    protected Claims decode(String jwtToken){
        // 得到 DefaultJwtParser
        return Jwts.parser()
                .setSigningKey(base64EncodedSecretKey)//设置签名密钥
                .parseClaimsJws(jwtToken)//设置需要解析的jwt
                .getBody();
    }
